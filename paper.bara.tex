...
The existing solutions are based exclusively on Voronoi diagrams, suffering from several problems.

First, to search for the correspondence between tunnels in individual snapshots, these solutions are using different clustering methods, which are generally very time and memory consuming.
...
The dynamic tunnel does not need to be connected with the active site or the surface all the time which gives the users the information about the void space also in snapshots without traditional tunnels.
In consequence, the user gets much better idea about the behavior of the void space which can be used by a ligand to reach or leave the active site.

In our proposed solution to these problems, we utilize the algorithms from sampling-based motion planning.
We have recently proposed a novel method for tunnel detection using sampling-based motion planning, namely using Rapidly Exploring Random Tree (RRT) method~\cite{vonasek2017tunnel}.
...
handles the dynamics without any need to cluster and match Voronoi diagrams between consecutive frames, which leads to results much faster and with less memory consumption.
...
In the frame $n$, the active site (red sphere) is not fully connected with a protein surface, but only to an internal cavity.
The cavity disconnects from the active site in the frame $n+1$ and remains disconnected also in the next frame $n+2$.
In the fourth frame $n+3$, the cavity connects with the protein surface.
}
\end{figure}
...
The path in the roadmap then relates to the motion in the workspace.
...
In~\cite{guieysse2008structure,cortes2010simulating}, RRT is used to compute exit pathways for flexible ligand leading from an active site to the surface of the protein.
...
uniformly generated samples towards the medial axis~\cite{amatoOBPRM} or its estimation~\cite{hollemanMAPRM}.
Another approach is to estimate difficult regions e.g. based on the number of collision-free samples around a given configuration~\cite{overmarsGauss,hsuBridge}.
...
Instead of increasing probability of sampling in all regions, e.g. along the whole medial axis, the tree has to guided through the space.
...
The void space (the collision-free region) $\CF^i \subseteq \C^i$ is formed by configurations, where the $\Sprobe$ can be placed without any collision with atoms, i.e.,  $\CF^i = \{q \in \C^i | \Sprobe(q) \cap \SS^i = \emptyset\}$.
...
The outer space  $\CG^i \in \C^i$ of the protein is defined by configurations, where a large sphere can be placed without any collision,
...
We assume that the position (configuration) of the active site $\qinit \in \CF^i$ in each snapshot is known in advance or it is known that the probe 
cannot be placed at the active site.
...
The atoms located on the surface of the molecule can be identified using \mbox{$\alpha$-shape}. 
Let $\SSA^i \subset \SS^i$ denote a set of spheres representing these atoms in snapshot $i$.
%The distance between a configuration $q \in \C^i$ and its nearest surface atom is referred to as $\dists(q)$.
Since all configurations describe the position of the probe in 3D space, {\textcolor{red} {all distance evaluations between two configurations, between a configuration and a sphere}} are measured using the 3D Euclidean metric, which is denoted as $\dist(\cdot,\cdot)$ in the rest of the paper.
...
To maintain the connectivity between the void space from the previous frame and the newly discovered one, the void space
has to be explored incrementally without adding non-reachable regions, such as cavities.
...
The required incremental detection of the void space can be achieved by the exploration principle of the RRT planner, as the RRT incrementally extends a tree of collision-free configurations.
...
Before a configuration $q$ is added to the tree, the distance between $q$ and the surface atoms is computed.
...
In Alg.~\ref{alg::main}, the second phase starts after $\alpha$ fraction of all iterations.
...
\caption{\label{fig::blocking1}
    ...
    Finally, two tunnels are detected (with the exit points {\textcolor{red} {$P_1$ and $P_2$)}}, but any tunnel is found in the region $A$ (d).
}
\end{figure}
...
After the tree reaches a surface atom, a new blocking sphere (red large circle) is added to the collision detection (a).
...
This region is approximated by the Voronoi vertices $\VV^i$ and the samples can be generated around them.
...
It is necessary to guide the growth of the tree by increasing the probability of sampling only around the Voronoi vertices that can be reached by the tree with a higher probability.
...
$\db$.
The upper value $\da$ ensures that the active Voronoi vertices are close to the boundary of the configuration tree, whilst the lower value $\db$ prevents to activate the Voronoi vertices that have already been approached by the tree.
After the active Voronoi vertices are updated, {\textcolor{red} {they are sorted according to their distance to the tree attempts to extend towards them}} (Alg.~\ref{alg::main}, line~\ref{alg::main::d}--\ref{alg::main::e}).
...
\caption{\label{fig::av}
    ...
    The active Voronoi vertices that are close to the tree are depicted in light yellow.
    The random samples are generated around these active vertices.
    The tree is attracted by the samples in the yellow regions (left). 
    The Voronoi vertices that have been already approached are depicted in light pink (right).
    The random samples are not generated around these vertices, so the tree is still attracted towards the unexplored regions.
}
\end{figure}
...
In the second phase, the tree is required to grow outside the protein and therefore, the random samples are generated from the whole configuration space.
...
After the tree approaches a given $v \in \VVA$ too close (less than the distance $\db$), the vertex is removed from $\VVA^i$ in the next update, and the random samples are not generated around it.
...
The exploration of the void space is limited by the planner resolution $\varepsilon$, {\textcolor{red}{so the random sam}}
The set $\VVA^i$ does not need to be updated in each iteration, because the tree is expanded only by the distance $\varepsilon$ in each iteration.
The update can be realized, e.g., after every addition of $k$ nodes to the tree.
...
In the classic expansion, the tree is expanded by a configuration $\qnear$ located on the line from  $\qnear$ to $\qrand$ in the distance $\varepsilon$ from $\qnear$ (b).
However, the tree cannot be expanded more towards $\qrand$ from $\qnew$ as it is blocked by an obstacle (atom).
By adding multiple nodes around $\qnew$, the tree can be expanded further to $\qrand$.
}
\end{figure}
...
After a node $\qnear$ for the expansion is determined, its neighborhood in the distance $\probe$ is sampled using $m$ samples in order to find a configuration $q'$ that is collision-free (at radius $\probe$) and maximally approaches the random configuration $\qrand$.
The resulting configuration $q'$ is added to the tree.
To further expand the tree, the neighborhood of $q'$ is searched again by randomly generating $m$ new samples.
...
The regression test prevents the expansion to regions that are already filled dense with other nodes of the tree~\cite{kalisiakBlossom}.
After a new node is added to the tree, the distance to the surface atoms is checked and if the node is too close to the surface, a new blocking sphere is introduced (Alg.~\ref{alg::expand}, line~\ref{alg::expand::bl}).
The distance to the surface atoms is checked only {\textcolor{red}{in the first phase}} 
...
This increases the chance that $\qnear$ is expanded by at least one new node, which is important especially in the narrow passages, as it allows the tree to grow through the narrow regions.
...
$\SB = \emptyset$\;
\For{$i = 1,\ldots,m$}{
    $q$ = random sample around $\qnear$ from $\sim N(\qrand,\probe)$\;
...
\If{$q' \ne \emptyset$}{
    ...
    \For{$i = 1,\ldots,m$}{
        ...
        \If{$r(q) \ge \probe$ {\bf and} $\dist(q,q_n) > \dist(\qnear, q)$}{
            addNodeToTree($\T, q$)\;
            addEdgeToTree($\T, (q',q)$)\;
            $d$ = distance to the nearest atom surface $\SSA^i$\;
...
\section{Evolution of the void space through the protein dynamics}

Detection of dynamic tunnels requires to track the evolution of the void space through the molecular dynamics simulation.
Besides the detection of the void space in each frame, i.e., representing it using by a set of consecutive spheres, it is necessary to maintain the relation between the detected void space in the consecutive frames.

The sequence of frames of protein dynamics is searched by the repeated construction of the collision-free tree and its transfer to the next frame.
...
Also the nodes that are collision-free at radius $\gprobe$ are removed, as these are located outside the protein (Fig.~\ref{fig::tp2}).
These nodes are added to the tree in the second phase of the void space detection in order to find the exit points.
They have to be removed in order to ensure that the detection of the void space in the next frame begins inside the protein.

The parents of the remaining nodes are changed to the corresponding nodes from the previous tree, so all surviving nodes now point to the corresponding node of the previous frame.
The remaining nodes, though they are mutually disconnected, are stored again in the tree structure.
As the RRT selected the nodes for expansion only using their distance to the random samples, the edge information is not necessary.
The transfer of the tree to to the next frame of molecular dynamics is listed in Alg.~\ref{alg::transfer}.
The illustration of the transfer process is depicted in Fig.~\ref{fig::transfer}.
...
\For{$q \in \T_{i-1}$}{
    \If(//is collision-free in the new frame?){$q \in \CF^{i}$}{
        \If{$r(q) \ge \probe$ {\bf and} $r(q) < \gprobe$ }{
            ...
            set parent of $\T_{i}.q$ to $\T_{i-1}.q$ \;
        }
...
\multicolumn{5}{c}{ frame $i$ \hskip 28pt frame $i+1$ }
\end{tabular}
}
\caption{\label{fig::transfer}
Illustration of the transfer of the detected void space to the next frame.
In the frame $i$, the void space consists of one component (nodes $1--7$).
After the pruning, the nodes leading to collisions with atoms are removed (nodes $5,6$).
The parents of the remaining nodes are set to the same nodes in the previous frame.
...
The algorithm for tracking the evolution of the void space in the molecular dynamics is listed in Alg.~\ref{alg::dynamic}.
The algorithm starts with the first frame in the configuration $\qinit \in \CF^1$ that represents the initial position of the active site.
...
After the tree pruning (Alg.~\ref{alg::transfer}), the parents of the surviving nodes point to the previous frame and the edges connecting the nodes are omitted (Fig.~\ref{fig::transfer}c, e.g., the edge from the node 1 to 2 is not used).
A frequent event in the molecular dynamic is the splitting and merging of the void space.
...
This splitting is achieved during the tree pruning.

To enable merging the void space, it is necessary to find which nodes of the tree in a given frame can be connected by a collision-free line.
For each node $q$ of the tree, the $k$-nearest neighbors are found and it is determined if $q$ and these neighbors can be connected by a collision-free line.
...
This allows us to define a graph describing the connections between various components from different frames.
...
After the void space is detected in all frames by Alg.~\ref{alg::dynamic}, the tunnels can be identified using the resulting graph $G$. 
...
Second, the active site does not exist in the given frame.
The active sites are inputs to the algorithm and when searching the tunnel for higher probe sizes, it may happen that the active site is not defined in the given frame.
...
Instead, $P_2$ indicates the end of the dynamic tunnel.
This tunnel is reconstructed by finding a path from $P_2$ to the first active site in the previous frames, that is reachable from $P_2$.
In this case, the dynamic tunnel consists of $P_2, C_1, C_0, A_1$. 
This dynamic tunnel evolved from $A_1$ of the frame $i$ to the exit point $P_2$ in the frame $i+1$.
}
\end{figure}
...
The tunnels were investigated for probe $\probe=1.4$~\AA.
...
The positions of the active sites were take from the result of the static tunnel detection (i.e, position of the
active site was not known for 2860 frames).
...
The second phase of the search was switched after 70~\% of the iterations, i.e. after 840 iterations.
Other parameters used for the computations are: $m=8$, $\da=5$~\AA, $\db=0.5$~\AA.
Therefore, in each iteration, up to $m=8$ new samples can be added to the tree, therefore in each frame, up to
$1200 \times 8 = 9600$ new nodes can be adde to tree.

The number of the tree nodes however do not rise during the computation, because many of the nodes are pruned (Alg.~\ref{alg::transfer}).
The size of the built tree in each frame is depicted in Fig.~\ref{fig::treesize}a.
Similarly, the number of components remain same in in most of the frames (Fig.~\ref{fig::treesize}b).
...
\caption{\label{tab::dyn} The number of frames with detected dynamic tunnels. The number of frames
    with static tunnels was detection by CAVER 3.0}
\begin{tabular}{ccccc}
\toprule
                              &   & \multicolumn{3}{c}{ Static tunnels}   \\
                              &    &  {\bf Exists} & {\bf Not exists} & {\bf Total } \\
\midrule                                     
\multirow{3}{*}{Dynamic tunnels} & {\bf Detected} & 1324 & 1027 & 2351 \\
                                 & {\bf Not detected} & 3816 & 1833 & 5649 \\
                                 & {\bf Total }       &  5140 & 2860 \\
\bottomrule                                 
\end{tabular}
\end{table}
...
the maximally allocated memory durin the computation was 1GB.
...
\renewcommand{\tabcolsep}{0pt}
\begin{tabular}{cccccc}
\includegraphics[width=0.17\textwidth]{fig/framed-0027-000-0026} &
\includegraphics[width=0.17\textwidth]{fig/framed-0027-000-0027} & 
\includegraphics[width=0.17\textwidth]{fig/framed-0033-001-0032} & 
\includegraphics[width=0.17\textwidth]{fig/framed-0033-002-0033label} & 
\includegraphics[width=0.17\textwidth]{fig/framed-0033-002-0032} & 
\includegraphics[width=0.17\textwidth]{fig/framed-0033-001-0033label} \\
\multicolumn{2}{c}{tunnel I, frames 26--27} &
\multicolumn{2}{c}{tunnel I, frames 32-33} &
\multicolumn{2}{c}{tunnel II, frames 32-33} \\
\end{tabular}
\caption{\label{fig::seq1}
    Examples of dynamic tunnels that evolve over two frames.
   In the frame 26-27, only one dynamic tunnel was detected, while in the 
   frames 32-33, two dynamic tunnels were found. These two tunnels lead in two different positions on the surface (P1 and P2)
}
}
\end{figure*}
...
The proposed void space detection is based on sampling-based motion planinng and therefore, the method is not deterministic.
The number of found dynamic tunnels can therefore vary between trials.
Behavior of the method is influenced by several parameters like the maximum number of iterations $\Imax$ or the number of
samples $m$ in the expansion step.
The proposed algorithm can be further influences by the parameters $\da$ and $\db$.
The performance of the sampling-based void space detection also depends on the properties of the proteins.
Protein structures vary in the number of atoms, roughness of their surface, size of the internal cavities etc. 
Therefore, there is no universal setting of the parameters and they have to be set considering a given protein.
...
Increasing $\Imax$ and $m$ would obviously lead to construction of bigger trees, but not always.
The growth of the tree inside the proteins is limited not only by the atoms, but also by the blocking spheres.
The regression test performed before adding a new sample to the tree ensures that the tree does not grow towards its own nodes.
...
Two most time-consuming methods is the collision detection and the nearest-neighbor search.
The complexity of the collision detection, when realized using hierarchical collision detection structures like OBB or AABB, is 
The proposed method frequently utilizes 
Two computational
The complexity of the proposed

Obviously, increasing these two parameters would lead to construction of bigger trees, but it also brings disadvantages.
Nearest-heighbor search is slower for the higher trees, even if it is realized using efficient data structures like KD-trees.
Before a new node is added to the tree, the regression test is made, which prevents the tree to grow too much close to other nodes.

Adjusting parameters is also necessary with the VD-based tools



As each frame of molecular dynamics is analyzed separately, only the actual configuration tree and frame-related data structures 
...
The sampling in the next frame therefore does not start from scratch, but it continues by adding new nodes to the existing tree.
The number of iterations $\Imax$ has to be large enough for the first frame in order to maximize the number of detected tunnels, but it can 
...
Besides that, a simple 3D Euclidean metric can be used to measure the distances in the configuration space.
Consequently, the nearest neighbor search can be realized using KD-trees in $\mathcal{O}(\log n)$, where $n$ is the number of 
nodes in the tree.
KD-trees can also be used to compute the distance $\dists$ to the surface atoms  TODO.
which makes these procedures fast.

   
The distance to the surface atoms $\ths$ indicating tunnel exit points depends on the roughness of the protein surface and we recommend
