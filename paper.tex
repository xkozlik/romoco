...
It is also necessary to remove the nodes outside the protein (these nodes are added in the second phase of the void space detection in order to find the exit points).
...
This allows us to define a graph $G$ of void space evolution.
...
%\setstretch{\straa}
{\small
\caption{Tunnel detection in protein dynamics
\label{alg::dynamic}
}
\KwIn{
    starting point $\qinit \in \CF$ in the first frame
}
\KwOut{list of configuration trees $\T_i$, list of exit points $P$, graph $G$ 
}
\hrule
$\T_0$.add($\qinit)$\;
...
\For{$i  = 1 : n$}{
    ...
    add components $C$ as nodes to graph $G$ \\
...
One tunnel is defined by the exit point and by the active site.
...
If there does not exist any path between the exit point $p \in P^i$ (the exit point at frame $i$) and the active site $\qinit \in \CF^i$ in the same frame, the dynamic tunnel has to start from a different active site (in one of the previous frames).
A path from $p \in P^i$ to the first active site $\qinit\in\CF^j, j < i$ reachable in the graph (measured by the number of edges) describes the dynamic tunnel.
...
As the positions of active site were taken from CAVER, which didn't find suitable place for the large probe in these frames,
...
A static tunnel as well as a dynamic one was detected in 1324 frames.
