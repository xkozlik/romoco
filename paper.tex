...
\For{$i  = 1 : n$}{
    ...
    $(\T_i, P^i)$ = voidSpaceDetection($\T_{i},\SB)$; //Alg.~\ref{alg::main}\\
    $P=P \cup P^i$\;
    ...
    connect exit points $P^i$ with their components (nodes) in $G$\;
...
If both the active site $\qinit\in\CF^i$ and the exit point $p \in P^i$ are connected to the same component, a static
tunnel is described by a path from $p$ to the active site.
Otherwise, the dynamic tunnel has to be reconstructed.
