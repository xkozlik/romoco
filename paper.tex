...
Namely due to the high memory consumption, these methods are not suitable for tunnel detection in molecular dynamics and therefore they are used primarily for the analysis of static molecules or individual snapshots of the molecular dynamics.
...
To overcome this, the weights of individual points can be determined by the van der Walls radii of the atoms in WVD.
An alternative solution is to compute a non-weighted VD on an extended point set, where each atom is approximated by several spheres with a same small radius~\cite{yaffe2008,caver3,caverDetails}.
VD-based methods are memory less demanding, and also faster than the grid-based methods.
The extension of the VD-based methods to molecular dynamics requires to construct the VD in all frames of the simulation and finding the correspondence over time between the tunnels detected in these frames.
...
Another problem of these approaches is the definition of the tunnel bottleneck threshold, which determines the minimal size of the narrowest tunnel point.
...
Alternatively, the tunnel detection in dynamic proteins can be formulated as a path planning problem in 4D (i.e., 3D $\times$ time) configuration space, assuming a spherical probe.
...
In the sampling-based motion planning, the configuration space is randomly sampled and the samples are classified as free or occupied using collision detection.
...
The advantage of sampling-based motion planners is that they can cope with many-DOF (Degrees Of Freedom) robots and due to the black-box collision detection, they can also cope with robots of arbitrary shapes.
...
In these applications, the molecules are represented as a kinematic chain where the joint angles correspond to bonds of the molecules.
...
In each iteration of the RRT, a random configuration $\qrand$ is generated and its nearest node $\qnear$ in the tree  is found.
...
In~\cite{guieysse2008structure,cortes2010simulating}, the RRT is used to compute exit pathways for a flexible ligand leading from the active site to the surface of the protein.
...
The RRT-ML expands the tree primarily using those DOF that are essential for achieving the motion of the ligand (i.e., rotation and translation) and it employs the other DOFs (i.e., those that are responsible for conformation changes) if they hinder the growth of the tree.
In~\cite{cortesMD}, the pathway is found for the opposite direction, from the active site towards the protein surface.
The methods~\cite{guieysse2008structure,cortesMD} do not compute all tunnels explicitly, rather a single path.
...
A possible solution is to estimate the location of the narrow passages from the knowledge of the workspace and generate more samples in the difficult areas.
The PRM-based planners can cope with the narrow passage problem by increasing the probability of sampling, e.g., around the medial axis~\cite{amatoOBRRT,amato2002using,wilmarthMAPRM}, or even by shifting the uniformly generated samples towards the medial axis~\cite{amatoOBPRM} or its approximation~\cite{hollemanMAPRM}.
Another possible approach is to estimate the difficult regions online, e.g., based on the number of the collision-free samples around a given configuration~\cite{overmarsGauss,hsuBridge}.
However, generating random samples in the difficult regions does not ensure the construction of a better tree in the case of RRT-based methods.
As the RRT builds the tree incrementally, the tree can connect to a region with increased probability of sampling only if the growth of the tree is not blocked by obstacles.
Instead of increasing the probability of sampling in all regions at once, e.g., along the whole medial axis, the tree has to be guided through the space to reach the regions gradually.
...
In~\cite{kardossRRTKK}, the samples are generated along several waypoints that are located close to the narrow passages.
...
Our proposed algorithm for the detection of dynamic tunnels works as follows.
...
After the void space is detected in a given frame, it is transferred to the next frame and pruned in order to remove collisions with the atoms in the next frame.
...
This corresponds to the tunnel detected by the traditional methods.
Otherwise, the detected void space is searched back in time to reconstruct our newly proposed dynamic tunnel.

The work presented in this paper is the extension of our previous work~\cite{vonasek2017tunnel}, which proposed to guide the search of the configuration space using the Voronoi vertices of the Voronoi-based protein representation.
Our goal was to detect static tunnels in proteins.
In this work, we focus on the detection of dynamic tunnels and their visual representation enabling the domain experts to further analyze the evolution and behavior of the protein void space.
...
For each configuration $q\in\C^i$ we assume that its largest collision-free radius $r(q) \in \mathbf{R}, r(q)\ge 0$ can be computed, e.g., using collision detection.
The outer space  $\CG^i \subset \CF^i$ of the protein is defined by configurations, where a large sphere can be placed without any collision, i.e., $\CG^i=\{q\in \C^i| \Sgprobe(q) \cap \SS^i = \emptyset \}$ where $\Sgprobe$ is the sphere of radius $\gprobe > \probe$ placed at the configuration $q$.
The exit points, where the tunnels leave the protein, are located in $\CG^i$.
To determine whether two configurations $q_1, q_2 \in \CF^i$ can be connected, a line segment from $q_1$ to $q_2$ is constructed and discretized to several points, using $\varepsilon$ discretization step, and the collision detection is performed at all the points.
The two configurations can be connected if all intermediate points on the line are collision-free. 
The 3D Euclidean metric is used to measure the distances in the configuration space.
...
A static tunnel is a collision-free path leading from $\qinit \in \CF^i$ to an exit point $q \in \CG^i$, i.e., both the active site and the exit point are located in the same frame $i$.
...
The second case is the result of the void space detection in the previous frame of the molecular dynamics.
...
The required incremental detection of the void space can be achieved by the exploration principle of the RRT planner, as RRT incrementally extends the tree of collision-free configurations.
...
\KwOut{list of configuration trees $\T_i$, list of exit points $P$, graph of components $G$
}
\hrule
$\T_0$.add($\qinit)$\;
...
\For{$i  = 1 : n$}{
    ...
    add components $C$ to graph $G$ as nodes\;
    connect exit points $P'$ with their components (nodes) in $G$\;
    connect active site $\qinit \in \CF^i$ with its component (node) in $G$\;
}
}
\end{algorithm}
...
A static tunnels is contained completely in one frame, therefore the path from $p \in P^i$ to $\qinit \in \CF^i$ in the graph
$G$ describes it.
Dynamic tunnels have active site and exit point in different frame, so to reconsruct them,
a path from $p \in P^i$ the first reachable $\qinit \in \CF^j, j < i$ has to be searched in the grapg $G$.
%If both these nodes are connected to the same component of the graph, they define a static tunnel.
%Such a tunnel can be reconstructed by finding a path between these two nodes in the graph.
...
\caption{\label{tab::dyn} The number of frames with detected dynamic tunnels. 
    %The number of frames with static tunnels, detected by CAVER 3.0.
}
\begin{tabular}{ccccc}
\toprule
  \multirow{2}{*}{ Proposed method}   &  \multicolumn{2}{c}{ Static tunnels}   \\
